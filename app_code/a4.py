...
def getAllStudents():
	...
	# Execute query to get all tuples in the 'students' table
...
def addStudent(first_name, last_name, email, enrollment_date):
	...
	# This except block is for catching UniqueViolation exceptions (when the email of the student we are trying to add is not unique)
		...
		# This rollback() function call deals with the case of a bad query (same with the rest of the rollback() calls)
		connector.rollback()
...
def updateStudentEmail(student_id, new_email):
	...
	try:
		...
		# Checking if student_id is a valid id
		c_obj.execute("SELECT * FROM students")
		output = c_obj.fetchall()
		...
		print('\n')
		print("The new email you chose is already used by another student and is therefore not unique. The email of the student wasn't updated.")
		connector.rollback()

# Deletes the record of the student with specified student_id from the 'students' relation
def deleteStudent(student_id):
	
	# Checking if student_id exists before deleting
	c_obj.execute("SELECT * FROM students")
	o = c_obj.fetchall()
